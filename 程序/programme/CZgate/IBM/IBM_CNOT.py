...
def getfid(T):
    ...
    U1 = basis(N,0)*basis(N,0).dag()+np.exp(1j*(E[1])*tlist[-1])*basis(N,1)*basis(N,1).dag()
    U2 = basis(N,0)*basis(N,0).dag()+np.exp(1j*(E[2])*tlist[-1])*basis(N,1)*basis(N,1).dag()
    UT = tensor(U1,U2)
...
#    axes[1].plot(tlist,l0);axes[1].set_xlabel('t');axes[1].set_ylabel('L1')
#==============================================================================
...
#    w8 = 'alpha*omega/2*(erf((t-tp-60-8)/5)-erf((t-tp-60-tp+8)/5))*(np.cos(f1*t+np.pi+0.5*np.pi))*(tp+60<t<=2*tp+60)'
    ...
    args = {'omega':omega,'tp':tp , 'ramp': 5 , 'f1':f1 , 'f2':f2, 'alpha':alpha , 'D':D}
    ...
    # p.close()
    # p.join()
    print(P,D,np.mean(leakage),fid,np.mean(fid))
    # gc.collect()


    
    # psi = [tensor(basis(3,0),basis(3,1)),(tensor(basis(3,0),basis(3,1))-1j*tensor(basis(3,0),basis(3,0))).unit()]
...
if __name__=='__main__':
    ...
    eta_q=  np.array([-1.330 , -1.330]) * 2 * np.pi
...
#    fid = CNOT([84.94258898,0.7914247])
#    print(fid)
    # x0 = [40,0.14*2*np.pi]
    # result = minimize(CNOT, x0, method="Nelder-Mead",options={'disp': True})
