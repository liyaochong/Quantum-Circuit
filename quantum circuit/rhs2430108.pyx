...
def cy_td_ode_rhs(
    ...
    spmvpy(data1, idx1, ptr1, vec, (w_t0)*(0<t<=40), out)
    spmvpy(data2, idx2, ptr2, vec, (w_t1)*(0<t<=40), out)
    spmvpy(data3, idx3, ptr3, vec, (Omega0*(np.exp(-(t-t1/2.0-0)**2/2.0/width0**2)*np.cos(t*f0)-(t-t1/2.0-0)/2.0/width0**2/5.02654824574*np.exp(-(t-t1/2.0-0)**2/2.0/width0**2)*np.cos(t*f0+np.pi/2.0)))*(0<t<=40), out)
    spmvpy(data4, idx4, ptr4, vec, (Omega1*(np.exp(-(t-t1/2.0-0)**2/2.0/width1**2)*np.cos(t*f1)-(t-t1/2.0-0)/2.0/width1**2/5.02654824574*np.exp(-(t-t1/2.0-0)**2/2.0/width1**2)*np.cos(t*f1+np.pi/2.0)))*(0<t<=40), out)
    return out
