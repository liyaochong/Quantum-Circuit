...
def cy_td_ode_rhs(
    ...
    spmvpy(data3, idx3, ptr3, vec, (Omega0*(np.exp(-(t-20-0)**2/2.0/width0**2)*np.cos(t*f0)-(t-20-0)/2.0/width0**2/1.25663706144*np.exp(-(t-20-0)**2/2.0/width0**2)*np.cos(t*f0+np.pi/2.0)))*(0<t<=40), out)
    spmvpy(data4, idx4, ptr4, vec, (Omega1*(np.exp(-(t-20-0)**2/2.0/width1**2)*np.cos(t*f1)-(t-20-0)/2.0/width1**2/1.25663706144*np.exp(-(t-20-0)**2/2.0/width1**2)*np.cos(t*f1+np.pi/2.0)))*(0<t<=40), out)
    return out
