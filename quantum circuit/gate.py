...
single qubit gate
'''
def Gate_rx(inxc,t0,t1,phi=np.pi,width = 6,setting = gatesetting()):
#    print(setting.DRAG)
    args_i={}
...
def Gate_ry(inxc,t0,t1,phi=np.pi,width = 6,setting = gatesetting()):
    args_i={}
    w_t_i='(w_t'+str(inxc)+')*('+str(t0)+'<t<='+str(t1)+')'
    if setting.DRAG:
    ...
    args_i['width'+str(inxc)]=width
    return w_t_i,D_t_i,args_i   
def Gate_rz(inxc,t0,t1,phi=np.pi,width = 6,setting = gatesetting()):
...
two qubits gate
'''
def Gate_iSWAP(inxc,inxt,t0,t1,phi=np.pi,width = 0.5,setting = gatesetting()):
...
def Gate_CZ(inxc,inxt,t0,t1,phi=np.pi,width = 10,setting = gatesetting()):
