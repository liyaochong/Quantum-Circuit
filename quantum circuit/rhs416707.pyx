...
def cy_td_ode_rhs(
    ...
    spmvpy(data3, idx3, ptr3, vec, 0*(0<t<=80), out)
    spmvpy(data4, idx4, ptr4, vec, (-Omega1/2*np.exp(-(t-20-0)**2/2.0/width1**2)*np.cos(t*f1))*(0<t<=40)(Omega1*np.exp(-(t-20-0-40)**2/2.0/width1**2)*np.cos(t*f1))*(40<t<=80), out)
    return out
